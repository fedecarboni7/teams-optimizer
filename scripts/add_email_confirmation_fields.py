    ...
    try:
        ...
        with engine.begin() as connection:
            ...
            if 'email_confirmed' not in columns:
                migrations_needed.append("ADD COLUMN email_confirmed BOOLEAN DEFAULT FALSE NOT NULL")
            ...
            # Set email_confirmed = TRUE for existing users who already have email
            # This assumes existing users with emails are already "verified"
            update_sql = """
                UPDATE users 
                SET email_confirmed = TRUE 
                WHERE email IS NOT NULL AND email != '' AND email_confirmed = FALSE
            """
